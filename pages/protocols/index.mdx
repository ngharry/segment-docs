...
![](https://segment.com/docs/protocols/tracking-plan/images/tracking-plan.png)
...
![](https://segment.com/docs/protocols/tracking-plan/images/labels.png)
...
> Ne[gative lookahead regexes (?!)](https://www.regular-expressions.info/lookaround.html) aren’t supported. This means you can’t use regex to prevent matches with a specific following character or expression. But, you can use not in the regex of your JSON schema to generate violations when a property key or value doesn’t match the provided regex pattern.

## Extend the Tracking Plan

Some customers prefer to manage the Tracking Plan with outside tools and resources. See the [APIs and extensions](https://segment.com/docs/protocols/apis-and-extensions/) section to learn more.
...
It’s best to keep traits optional because `.identify()` and `.group()` are often called and pass only _new or changed_ traits, because Segment’s client-side libraries (analytics.js, iOS, Android) cache traits in local storage. See the [Identify Best Practices](https://segment.com/docs/connections/spec/best-practices-identify#when-and-how-often-to-call-identify) to learn more.
...
![](https://segment.com/docs/protocols/images/breaking_change_event_versioning.png)

Before Segment introduced event versioning, you would need to add the change to your tracking plan and any non-compliant events would generate violations, and possibly be blocked depending on your [event blocking settings.](https://segment.com/docs/protocols/enforce/schema-configuration/)
...
![](https://segment.com/docs/protocols/images/add_event_version.png)
...
![](https://segment.com/docs/protocols/images/pull_event_version.png)

Next, add the event version number to the context object. For [analytics.js](https://segment.com/docs/connections/sources/catalog/libraries/website/javascript) Track calls, you would instrument the event as in the example below. Note how the JSON objects for `context, protocols, and event_version` are nested.
